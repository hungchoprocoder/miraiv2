...
•Ví dụ: file help`;
    ...
    if(files.length == 0) return api.sendMessage(`Không có file nào trong thư mục commands có ký tự bắt đầu bằng: ${word}`, event.threadID ,event. messageID);
  ...
  //đuôi file là..... 
    ...
    if(files.length == 0) return api.sendMessage(`Không có file nào trong thư mục commands có ký tự kết thúc bằng: ${ext}`, event.threadID ,event. messageID);
    var key = `Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage("Thư mục commands của bạn không có file hoặc folder nào", event.threadID ,event. messageID);
  ...
  //trong tên có ký tự.....
    ...
    files.forEach(file => {
      var checkDir = fs.statSync(__dirname+'/'+file);
      var typef;
      checkDir.isDirectory() ? typef = "[Folder🗂️]" : typef = "[File📄]";
      msg += (i++)+'. '+typef+' '+file+'\n';
    });
